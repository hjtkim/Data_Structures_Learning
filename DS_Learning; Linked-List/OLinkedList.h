...
int ListFirst(List *plist, Data *pdata); // 첫 번째 데이터를 읽어오는 함수. 성공시 True를 반환하고, pdata에 데이터를 저장.
int ListNext(List *plist, Data *pdata); // 현재 위치에서 다음 데이터를 읽어오는 함수.
Data ListRemove(List *plist); // 현재노드를 제거하는 함수. 제거된 노드의 데이터를 반환함.
int ListCount(List *plist); // 리스트에 저장된 데이터의 개수를 반환하는 함수.
